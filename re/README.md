...
基于 Browserbase 远程浏览器服务和自建域名的 cf_temp_mail 的 Codex Token 自动注册工具。
...
```json
{
  "cfToken": "你的cf_temp_mail的管理员密码",
  "APIUrl": "你的cf_temp_mail的后端地址",
  "domain": "你的temp_mail中@后面的部分",
  "mailUrl": "你的cf_temp_mail的前端地址",
...
| 字段                  | 说明                                             | 必填                              |
| --------------------- | ------------------------------------------------ | --------------------------------- |
| `cfToken`           | cf_temp_mail 服务中的管理员密码                 | ✅                                |
| `APIUrl`            | cf_temp_mail 服务中后端根域名（可通过抓包得到） | ✅                                |
| `oauthClientId`     | OAuth 客户端 ID                                  | ❌ 默认即可                       |
| `oauthRedirectPort` | 本地回调端口                                     | ❌ 默认 1455 （其实根本不会使用） |
| `domain`            | cf_temp_mail 服务中邮箱的域名（@后面的部分）    | ✅                                |
| `mailUrl`           | cf_temp_mail 服务中前端根域名                   | ✅                                |
