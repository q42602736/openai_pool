...
let HttpsProxyAgentCached = null;

async function getHttpsProxyAgent() {
    if (!HttpsProxyAgentCached) {
        const mod = await import('https-proxy-agent');
        HttpsProxyAgentCached = mod.HttpsProxyAgent;
    }
    return HttpsProxyAgentCached;
}
...
class BrowserbaseService {
     ...
     */
    sendCDPCommand(method, params = {}, timeoutMs = 5000) {
    ...
    /**
     * 在当前页面上执行 JavaScript 表达式
     * @param {string} expression - JS 表达式
     * @param {object} options
     * @param {number} options.timeout - 超时（毫秒）
     * @returns {Promise<any>} - 表达式返回值
     */
    async evaluateOnPage(expression, options = {}) {
        ...
        if (result?.exceptionDetails) {
            throw new Error(result.exceptionDetails.text || 'Runtime.evaluate 异常');
        }
    ...
    connectToCDP(wsUrl, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            const handleObservedUrl = (currentUrl) => {
                ...
                if (typeof rejectMatcher === 'function' && rejectMatcher(currentUrl)) {
                    console.log(`[Browserbase] 检测到拒绝条件，终止监控`);
                    settleReject(new Error(rejectMessage || `检测到拒绝条件: ${currentUrl}`));
                    return true;
                }
            ...
            const connect = async () => {
                ...
                // 清理旧连接：防止 pollTimer 累积和旧 WebSocket 泄漏
                if (pollTimer) {
                    clearInterval(pollTimer);
                    pollTimer = null;
                }
                if (this.wsConnection) {
                    ...
                    this.clearPendingCommands('重连前清理旧连接');
                ...
                this.wsConnection = new WebSocket(fullWsUrl, wsOpts);
